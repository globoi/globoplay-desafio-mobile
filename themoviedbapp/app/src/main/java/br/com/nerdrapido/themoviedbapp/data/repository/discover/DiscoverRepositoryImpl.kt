 ...
 */
class DiscoverRepositoryImpl(retrofit: Retrofit) : AbstractMovieDbApiRepos(retrofit),
    DiscoverRepository {

    private val authService: DiscoverService = retrofit.create(DiscoverService::class.java)

    override suspend fun loadDiscover(discoverRequest: DiscoverRequest): ResponseWrapper<DiscoverResponse> {
        return safeApiCall(dispatcher) {
            authService.discoverMovies(
