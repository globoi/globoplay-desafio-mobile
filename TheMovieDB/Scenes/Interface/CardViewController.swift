...
class CardViewController: UIViewController , UIScrollViewDelegate {
    ...
    func loadRecomendationsData() {
        TheMovieDBAPI.sharedInstance.getTVSimilars(card: self.card) { (cards: [Card], error: String?) in
    ...
    func loadDataDetails() {
        if self.card.type == CardType.MOVIE.rawValue {
            TheMovieDBAPI.sharedInstance.getMovieDetail(id: Int(self.card!.id)) { (card: Card, error: String?) in
               if card.id != 0 {
                   self.card = card
               }
    ...
    func configLayout() {
        ...
        // Imagem de tras
        self.backImageView = UIImageView(frame: CGRect(x: 0, y: -65, width: screenWidth, height: screenWidth))
        ...
        let titleHeight : CGFloat = Functions.heightForView(text: self.card.name, font: font, width: screenWidth-margin*2)
        ...
        let descriptionHeight : CGFloat = Functions.heightForView(text: self.card.overview, font: fontDescription, width: screenWidth-margin*2)
        ...
        self.buttonWatch = UIButton(frame: CGRect(x: margin, y: buttonOrigin, width: buttonWidth, height: 44))
        ...
        // botao de favoritos
        self.buttonFavorite = UIButton(frame: CGRect(x: screenWidth/2+margin, y: buttonOrigin, width: buttonWidth, height: 44))
    ...
    func checkAlreadyFavorited() {
        ...
        if (TheMovieDBAPI.sharedInstance.checkAlreadyAdded(card: self.card)) {
    ...
    @objc func addFavorite() {
        if (TheMovieDBAPI.sharedInstance.checkAlreadyAdded(card: self.card)) {
            ...
            Functions.showChoiceCallbackAlert(controller: self, title: "Tem certeza?", message: message) {
                TheMovieDBAPI.sharedInstance.removeFavorite(card: self.card)
                self.checkAlreadyFavorited()
            }
        } else {
            TheMovieDBAPI.sharedInstance.addFavorite(card: self.card)
            self.checkAlreadyFavorited()
        }
    }
