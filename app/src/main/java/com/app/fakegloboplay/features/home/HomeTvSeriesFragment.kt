...
class HomeTvSeriesFragment : BaseFragment() {
                ...
                addOnScrollListener(object : RecyclerView.OnScrollListener() {
                    override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                        ...
                        if (!recyclerView.canScrollHorizontally(1)) {
                            println(currentPage)
                            homeViewModel.getTvPopular(currentPage)
                        }
                    }
                })
            }
            ...
            listPopularState.observe(viewLifecycleOwner) { state ->
                when (state) {
                    is ListMoviePageViewState.Success -> {
                        hideLoading()
                        popularAdapter.submitList(state.listMovie.results)
                        currentPage = state.listMovie.nextPage()
                    }
